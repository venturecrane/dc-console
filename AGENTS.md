...
venturecrane/dc-console - DurganCo operator console.
...
2. Call `crane_sos` with `venture: "dc"` - initializes session, shows P0 issues, cadence briefing, active sessions.
...
Mechanical enforcement status: dc-console's `eslint.config.js` does NOT yet enforce the portfolio rule set; lint catches only the basic recommended rules. Self-discipline is required until the per-venture eslint adoption initiative reaches dc-console (audit doc: `docs/research/venture-eslint-adoption-audit-2026-05-06.md` in venturecrane/crane-console). The portfolio rules are nonetheless mandatory; treat the global doc as the source of truth.

## Enterprise Rules

- All changes go through PRs. Never push directly to main.
- Work only on issues assigned to the current venture context (dc).
- If you detect scope drift (working on a different repo than session context), stop and verify with the user.
- All GitHub issues created this session must target `venturecrane/dc-console`.
